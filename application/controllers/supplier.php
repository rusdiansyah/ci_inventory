...
class Supplier extends CI_Controller {
	...
	public function index()
	{
		...
		if(!empty($cek)){
			...
			$text = "SELECT * FROM supplier $where ";		
	...
	public function edit()
	{
		...
		if(!empty($cek)){
			...
			$text = "SELECT * FROM supplier WHERE kode_supplier='$id'";
			$data = $this->app_model->manualQuery($text);
			if($data->num_rows() > 0){
				foreach($data->result() as $db){
					$d['kode']		=$id;
					$d['nama_supp']	=$db->nama_supplier;
					$d['alamat']	=$db->alamat;
				}
			}else{
					$d['kode']		=$id;
	...
	public function hapus()
	{
		...
		if(!empty($cek)){			
			$id = $this->uri->segment(3);
			$this->app_model->manualQuery("DELETE FROM supplier WHERE kode_supplier='$id'");
	...
	public function simpan()
	{
		...
		if(!empty($cek)){
				...
				$up['kode_supplier']	= $this->input->post('kode');
				$up['nama_supplier']	= $this->input->post('nama_supp');
